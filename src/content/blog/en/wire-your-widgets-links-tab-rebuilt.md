---
title: 'Wire your widgets: one surface, one sentence, and a Test it button'
description: 'Everything a widget can be wired to now lives behind one search box — other widgets, connections, bots and agents. Describe the wiring in plain language, and fire a real event through it to find out which end is broken.'
pubDate: 2026-08-08
heroImage: ../../../assets/blog/wires.svg
tags: ['product', 'widgets', 'automation']
---

A **link** carries events between two widgets: change the symbol in one and the
...
marks what you are already wired to, so a second pick reads as *"already
linked"* instead of silently repeating the first. Picking it again updates the
existing wiring rather than adding a duplicate row, and the badge says so
**before** you pay for a rebuild.

## "Test it": which end is actually broken?

Until this release a link was write-only. You described it, paid for the
...
wrong — the widget not emitting, the link not mapping, or the other widget not
listening.

The editor now has a **Test it** bar. Pick a topic and a value, choose which end
is pretending to emit (offered only on a two-way link, since a one-way link
would always report nothing from the far side), and send. The event goes into
the link's real runtime and runs the generated `connect(ctx)` — not a simulation
of it — and the verdict distinguishes every way it can come up empty:

- **This link isn't running.** Switch it on — or it has no code yet.
...
is armed only while the bar is on screen, so a link forwarding a tick stream
never pays for the instrumentation.

## The quiet ones

Two-way links would echo forever without help, so a delivered value is
remembered and the identical bounce-back is dropped once — the canonical A↔B
mirror loop breaks, while a genuinely new value still gets through. Links and
attachments replicate across your tabs and devices rather than living in one of
them, so deleting one elsewhere now shows a **gone** state in the editor instead
of letting you save into nothing. And when a rebuild fails, it says so and
points at the Builder tab, rather than leaving a spinner where an answer should
be.

One more change is on the way: the app is being renamed around the word people
already use for this. *Link* becomes **wire**, and *My wiring* stays exactly
where it is.
